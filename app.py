...
def init_db():
    ...
    if count == 0:
        samples = [
            ...
            ("과학탐구", "2026학년도 4월 과학탐구 모의고사", "2026", "4월", "개념 이해와 자료 해석 능력을 함께 평가하는 과학탐구 모의고사입니다.", 0, None),
...
def home():
    ...
    math_exams = [exam for exam in exams if exam["subject"] == "수학"]
    science_exams = [exam for exam in exams if exam["subject"] == "과학탐구"]
...
def admin_create():
    ...
    db.execute(
        ...
        VALUES (?, ?, ?, ?, ?, 0, ?)
        """,
        (subject, title, year, exam_month, description, pdf_filename),
    )
    db.commit()
